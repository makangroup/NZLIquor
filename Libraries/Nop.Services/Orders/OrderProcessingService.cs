...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        private readonly IWindcavePaymentService _windcavePaymentService;
        private readonly ICreditOrderPaymentService _creditOrderPaymentService;
        ...
        public OrderProcessingService(CurrencySettings currencySettings,
            ...
            IWindcavePaymentService windcavePaymentService,
            ICreditOrderPaymentService creditOrderPaymentService)
        {
            ...
            _windcavePaymentService = windcavePaymentService;
        ...
        protected virtual void SendNotificationsAndSaveNotes(Order order)
        {
            ...
            //If payment method is pay in store send order placed customer notification
            if (order.PaymentMethodSystemName == "Payments.PayInStore")
            {
                var orderPlacedAttachmentFilePath = _pdfService.PrintOrderToPdf(order);
                var orderPlacedAttachmentFileName = "Order.pdf";
                var orderPlacedCustomerNotificationQueuedEmailIds = _workflowMessageService.SendOrderPlacedCustomerNotification(order, order.CustomerLanguageId, orderPlacedAttachmentFilePath, orderPlacedAttachmentFileName);
            ...
            else if (order.PaymentMethodSystemName == "Payments.Credit")
            {
                var orderPaidAttachmentFilePath = _pdfService.PrintOrderToPdf(order);
                var orderPaidAttachmentFileName = "Order.pdf";
                var orderPaidCustomerNotificationQueuedEmailIds = _workflowMessageService.SendCreditOrderCustomerNotification(order, order.CustomerLanguageId, orderPaidAttachmentFilePath, orderPaidAttachmentFileName);
                
                if (orderPaidCustomerNotificationQueuedEmailIds.Any())
                    AddOrderNote(order, $"\"Order placed\" email (to customer) has been queued. Queued email identifiers: {string.Join(", ", orderPaidCustomerNotificationQueuedEmailIds)}.");
        ...
        /// </summary>
        /// <param name="order">Order</param>
        public virtual void ProcessCreditOrder(Order order)
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));
            ...
            if (order.OrderTotal != decimal.Zero)
            {
                ...
                var orderPaidAttachmentFilePath = _pdfService.PrintOrderToPdf(order);
                var orderPaidAttachmentFileName = "Order.pdf";
                var orderPaidCustomerNotificationQueuedEmailIds = _workflowMessageService.SendCreditOrderCustomerNotification(order, order.CustomerLanguageId,
                    orderPaidAttachmentFilePath, orderPaidAttachmentFileName);
        ...
        public virtual IEnumerable<int> ProcessCreditOrderPayment(string response)
        {
            var ids = new List<int>();
            var result = new ProcessPaymentResult();

            //Extract the data from the Windcave response
            (var responseOutput, var fileName) = _windcavePaymentService.ProcessResponse(response);
            if (responseOutput.valid == "1" && responseOutput.Success == "1")
            {
                //Update the CreditOrderPayment
                int creditOrderId = int.Parse(responseOutput.MerchantReference);
                var creditOrder = _creditOrderPaymentService.GetCreditOrderPaymentById(creditOrderId);
                creditOrder.FileName = fileName;
                creditOrder.LastUpdatedDate = DateTime.Now;
                _creditOrderPaymentService.Update(creditOrder);

                //Update the order status
                var merchantReference = _creditOrderPaymentService.GetCreditOrderPaymentById(int.Parse(responseOutput.MerchantReference));
                int[] orderIds = JsonConvert.DeserializeObject<int[]>(merchantReference.OrderIds);
                var orders = _orderService.GetOrdersByIds(orderIds);
        ...
        /// <summary>
        /// Places a Credit order
        /// </summary>
        /// <param name="processPaymentRequest">Process payment request</param>
        /// <returns>Place order result</returns>
        public virtual PlaceOrderResult PlaceCreditOrder(ProcessPaymentRequest processPaymentRequest)
        {
            if (processPaymentRequest == null)
                throw new ArgumentNullException(nameof(processPaymentRequest));
            ...
            try
            {
                ...
                if (processPaymentResult.Success)
                {
                    ...
                    if (order.PaymentStatus == PaymentStatus.Credit)
                        ProcessCreditOrder(order);
                }
                else
                    foreach (var paymentError in processPaymentResult.Errors)
                        result.AddError(string.Format(_localizationService.GetResource("Checkout.PaymentError"), paymentError));
