...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderTotalCalculationService : IOrderTotalCalculationService
    {
        ...
        public OrderTotalCalculationService(CatalogSettings catalogSettings,
            ...
            ICorporateCustomerService corporatecutomers)
        {
        ...
        public virtual bool IsFreeShipping(IList<ShoppingCartItem> cart, decimal? subTotal = null)
        {
            ...
            var customer2 = _customerService.GetAllCorporateCustomers();
            ...
            //if (customer.Id == _corporatecutomers.GetCustomerByCorporateCustomerId(cart.FirstOrDefault()?.CustomerId ?? 0).Id)
            //{
            //    if (subTotal.Value > _shippingSettings.FreeShippingNormalCustomerOverXValue)
            //        return true;-
            //}
          //  var corporatecustomer = _corporatecutomers.GetCustomerByCorporateCustomerId(customer1);
          foreach(var customercop in customer2)
            {
                if(customercop.Id == customer.Id)
                {
                    if (subTotal.Value > _shippingSettings.FreeShippingCorporateCustomerOverXValue)
                     return true;
                }
            }
