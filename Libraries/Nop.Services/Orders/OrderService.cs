...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderService : IOrderService
    {
        ...
        public virtual IPagedList<Order> SearchOverDueOrders(DateTime? createdFromUtc = null,
                                                             ...
                                                             bool getOnlyTotalCount = false)
        {
            ...
            //Select all over due orders
            query = from o in query
                    join copCust in _corporateCustomerRepository.Table
                    on o.CustomerId equals copCust.Customer_Id
                    where o.CreatedOnUtc < DateTime.Now.AddDays(-copCust.CreditPeriod)
                    select o;

            if (createdFromUtc != null)
                query = query.Where(o => o.CreatedOnUtc >= createdFromUtc);

            if (createdToUtc != null)
                query = query.Where(o => o.CreatedOnUtc <= createdToUtc);
        ...
        public decimal GetCreditOrdersTotal(Customer customer, int[] orderIds)
        {
            if (customer == null)
            ...
            return query.Sum(o => o.OrderTotal);
        }

        public virtual decimal GetCustomerOverDueAmount(int customerId)
        {
            if (customerId == 0)
                throw new ArgumentOutOfRangeException(nameof(customerId));


            decimal query = _orderRepository.Table.Where(o => o.CustomerId == customerId && o.PaymentStatusId == (int)PaymentStatus.Credit)
                                                  .Sum(s => s.OrderTotal);
