...
namespace Nop.Services.Messages
{
    ...
    public partial class WorkflowMessageService : IWorkflowMessageService
    {
        ...
        public virtual int SendCustomerFeedBackMessage(string sendToEmail, string CustomerName, string body, string subject, List<Token> tokens, int languageId)
        {
            var store = _storeContext.CurrentStore;
            languageId = EnsureLanguageIsActive(languageId, store.Id);

            var messageTemplate = GetActiveMessageTemplates("FeedbackAfter4Days.CustomerNotification", store.Id).FirstOrDefault();
            ...
            return SendNotification(messageTemplate, emailAccount, languageId, tokens, sendToEmail, CustomerName);
        }
        ...
        /// <param name="attachmentFileName">Attachment file name. If specified, then this file name will be sent to a recipient. Otherwise, "AttachmentFilePath" name will be used.</param>
        /// <returns>Queued email identifier</returns>
        public virtual IList<int> SendCreditOrderCustomerNotification(Order order, int languageId,
            string attachmentFilePath = null, string attachmentFileName = null)
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));
            ...
            var messageTemplates = GetActiveMessageTemplates(MessageTemplateSystemNames.CreditOrderPaidCustomerNotification, store.Id);
            if (!messageTemplates.Any())
                return new List<int>();
