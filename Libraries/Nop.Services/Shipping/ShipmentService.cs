...
namespace Nop.Services.Shipping
{
    ...
    public partial class ShipmentService : IShipmentService
    {
        ...
        /// <summary>
        /// Gets the List of shipments between two dates
        /// </summary>
        /// <param name="DeliveryDateUtcFrom">Delievry From Date</param>
        /// <param name="DeliveryDateUtcTo">Delievery To Date</param>
        /// <returns></returns>
        public virtual List<Shipment> GetShipmentsBetweenDates(DateTime DeliveryDateUtcFrom, DateTime DeliveryDateUtcTo)
        {
            var query = _shipmentRepository.Table;

            query = query.OrderByDescending(s => s.CreatedOnUtc);

            query = query.Where(s => DeliveryDateUtcFrom <= s.DeliveryDateUtc);

            query = query.Where(s => DeliveryDateUtcTo >= s.DeliveryDateUtc);
