...
namespace Nop.Web.Factories
{
    ...
    public partial class ShoppingCartModelFactory : IShoppingCartModelFactory
    {
        ...
        protected virtual ShoppingCartModel.ShoppingCartItemModel PrepareShoppingCartItemModel(IList<ShoppingCartItem> cart, ShoppingCartItem sci)
        {
            ...
            {
                decimal shoppingCartUnitPriceWithDiscount = decimal.Zero;
                if (!_profitMarkupsService.GetProductPrice(_workContext.CurrentCustomer.Id, product.Id, out shoppingCartUnitPriceWithDiscount))
                {
        ...
        protected virtual WishlistModel.ShoppingCartItemModel PrepareWishlistItemModel(ShoppingCartItem sci)
        {
            ...
            else if (_profitMarkupsService.GetProductPrice(_workContext.CurrentCustomer.Id, product.Id, out unitPrice))
            {
                cartItemModel.UnitPrice = _priceFormatter.FormatPrice(unitPrice);
            }
            else
            {
                var shoppingCartUnitPriceWithDiscountBase = _taxService.GetProductPrice(product, _shoppingCartService.GetUnitPrice(sci), out var _);
            ...
            if (_workContext.CurrentCustomer.HasShoppingCartItems)
            {
                ...
                if (cart.Any())
                {
                        ...
                        .ToList())
                    {
                        ...
                        {
                            var profitMarkup = _profitMarkupsService.Find(_workContext.CurrentCustomer.Id, product.Id);
                            ...
                            else 
                            {
                                ...
                                var finalPriceWithDiscount = product.ProductCost + profit;
