    ...
    public partial class CheckoutModelFactory : ICheckoutModelFactory
    {
        ...
        protected virtual CheckoutPickupPointsModel PrepareCheckoutPickupPointsModel(IList<ShoppingCartItem> cart)
        {
            ...
            if (model.AllowPickupInStore)
            {
                ...
                _orderTotalCalculationService.GetShoppingCartSubTotal(cart, true, out var _, out var _, out var subTotalWithoutDiscount, out var subTotalWithDiscount);
                ...
                //If there is not shipping providers or order amount is less than 50 display pickup store 
                if (!shippingProviders.Any() || subTotalWithoutDiscount <= 50)
                {
                    if (!pickupPointProviders.Any())
                    {
