...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class OrderModelFactory : IOrderModelFactory
    {
        ...
        protected virtual void PrepareAddressModel(AddressModel model, Address address)
        {
            ...
            model.CustomerNoteEnabled = true;
        ...
        public OverDueOrderListModel PrepareOverDueOrderListModel(OverDueOrderSearchModel searchModel)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));

            var orders = _orderService.SearchOverDueOrders(
                createdFromUtc: searchModel.StartDate,
                createdToUtc: searchModel.EndDate,
                pageIndex: searchModel.Page - 1, pageSize: searchModel.PageSize);

            //prepare list model
            var model = new OverDueOrderListModel().PrepareToGrid(searchModel, orders, () =>
            {
                //fill in model values from the entity
                return orders.Select(order =>
                {
                    var billingAddress = _addressService.GetAddressById(order.BillingAddressId);
                    var companyNameAttribute = _genericAttributeService.GetAttribute(order.CustomerId, NopCustomerDefaults.CompanyAttribute, nameof(Customer));

                    //fill in model values from the entity
                    var orderModel = new OverDueOrderModel
                    {
                        Id = order.Id,
                        DPD = (int)(DateTime.Now - order.CreatedOnUtc).TotalDays,
                        CompanyName = companyNameAttribute != null ? companyNameAttribute.Value : string.Empty,
                        Email = billingAddress.Email,
                        PhoneNo = billingAddress.PhoneNumber,
                        CreateOn = order.CreatedOnUtc,
                        ContactPersonName = $"{billingAddress.FirstName} {billingAddress.LastName}",
