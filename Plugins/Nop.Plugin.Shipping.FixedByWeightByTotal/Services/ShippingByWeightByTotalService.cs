    ...
    public partial class ShippingByWeightByTotalService : IShippingByWeightByTotalService
    {
            ...
            int countryId, int stateProvinceId, string zip, decimal? weight, decimal? orderSubtotal, int pageIndex, int pageSize)
        {
            ...
            var matchedByZip = string.IsNullOrEmpty(zip)
                ...
                : matchedByStateProvince.Where(r =>
                {
                    if (string.IsNullOrEmpty(r.Zip))
                        return true;

                    int zipCode = Int32.Parse(zip);
                    int[] zipCodes = r.Zip.Split("-").Select(Int32.Parse).ToArray();
                    if(zipCodes.Length == 2 && zipCode > zipCodes[0] && zipCode < zipCodes[1])
                        return true; //if the zip code is a rage split by "-"
                    else if (zipCodes.Length == 1 && zipCode == zipCodes[0])
                        return true; //if the zip code is only one item.
